...
def listening_scripts_maker(level, theme, bot_id=bot_id, url=url, headers=headers):
    prompt = "Hãy tưởng tượng bạn là một giáo viên đang kiểm tra kỹ năng nghe tiếng anh của học sinh. Bạn hãy tạo ra một kịch bản bài nói bằng tiếng anh để kiểm tra kỹ năng kỹ năng đọc ở mức trình độ " + \
        str(level)+" về chủ đề "+str(theme) + \
        ". Hãy chỉ trả về đoạn nội dung kịch bản bài nói và không trả về các câu hỏi, tiêu đề hay bất kỳ thứ gì thừa thãi khác không liên quan tới nội dung đoạn kịch bản nói."
    data = {
        "conversation_id": "123",
        "bot_id": bot_id,
        "user": "123333333",
...
def listening_question_maker(level, essay, bot_id=bot_id, url=url, headers=headers):
    prompt = "Hãy tưởng tượng bạn là một giáo viên đang kiểm tra kỹ năng nghe tiếng anh của học sinh. Kịch bản bài nghe:"+str(essay) + \
        ". Hãy tạo ra 5 câu hỏi liên quan tới các thông tin trong bài nghe trên với mức trình độ " + \
        str(level)+". Chỉ trả về các câu hỏi được cách dòng với nhau và không trả về bất cứ thứ gì không liên quan. Sau đây là bài viết của học sinh: "+str(essay)
    data = {
        "conversation_id": "123",
        "bot_id": bot_id,
        "user": "123333333",
...
def listening_question_grader(level, question, essay, answer, bot_id=bot_id, url=url, headers=headers):
    prompt = "Hãy tưởng tượng bạn là một giáo viên đang kiểm tra kỹ năng nghe tiếng anh của học sinh. Kịch bản bài nghe là: "+str(essay) + \
